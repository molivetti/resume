...
{July 2022 - Current} % Date(s)
{
  \begin{cvitems} % Description(s) of tasks/responsibilities
    ...
    \item {\keyword{Achieved ~200× growth in enrolled client debt} — from millions to over \$1B in under three years — by leading frontend initiatives that helped establish the company as an industry leader.}
    \item {\keyword{Spearheaded development of a Next.js monorepo} using React, Turborepo, Tailwind, TypeScript, Formik, Cypress, and Jest, including:}
    \begin{itemize}[leftmargin=2.5ex]
      \vspace{.7mm}
      \setlength{\itemsep}{.9mm}
      \item {Architected a highly scalable system enabling \keyword{infinite site generation with customizable themes and content} via ButterCMS.}
      ...
      \item {Extended ButterCMS functionality with \keyword{dynamic content injection}, including personalized user and geolocation data.}
      ...
      \vspace{.5mm}
    \end{itemize}
    \item {\keyword{Led the migration} from a legacy Ruby on Rails app to Next.js, streamlining performance and modernizing the stack.}
    \item {Managed \keyword{all critical third-party integrations}, including Google Tag Manager, Facebook Pixel, TikTok, Cookiebot, and custom analytics cookies for user and session tracking.}
  \end{cvitems}
}
